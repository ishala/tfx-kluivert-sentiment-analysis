...
def get_vocab_size(tf_transform_output):
    ...
    vocab_path = os.path.join(tf_transform_output.transform_savedmodel_dir, "assets", "vocab")
...
def count_samples(file_patterns, tf_transform_output):
    dataset = input_fn(file_patterns, tf_transform_output, num_epochs=1, batch_size=1)
    sample_count = sum(1 for _ in dataset)
    if sample_count == 0:
        raise ValueError("Dataset is empty. Check the file patterns and data source.")
    return sample_count
...
def input_fn(file_pattern, tf_transform_output, num_epochs, batch_size=64) -> tf.data.Dataset:
    ...
    if not any(dataset):
        raise ValueError("Dataset is empty. Check the file pattern and data source.")
...
def tuner_fn(fn_args: FnArgs) -> TunerFnResult:
    ...
    tf_transform_output = tft.TFTransformOutput(fn_args.transform_graph_path)  # Load transform output
    ...
    if steps_per_epoch == 0 or validation_steps == 0:
        raise ValueError("Steps per epoch or validation steps cannot be zero. Check the dataset.")
