    ...
    eval_config = tfma.EvalConfig(
        ...
        metrics_specs=[
            tfma.MetricsSpec(metrics=[
                ...
                tfma.MetricConfig(class_name='AUC', config=json.dumps({'curve': 'ROC'})),
                ...
                threshold=tfma.MetricThreshold(
                    value_threshold=tfma.GenericValueThreshold(lower_bound={'value': 0.1})  # Lebih rendah
                    )
                ),
    ...
    pusher = Pusher(
        ...
        push_destination=pusher_pb2.PushDestination(
            filesystem=pusher_pb2.PushDestination.Filesystem(
                base_directory=os.path.join(serving_model_dir, 'sentiment-analysis-model'))
        )
    )
